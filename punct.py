...
EDGE_SEARCH_HALF_MM = 1.5        # cat cautam sus/jos de linia mediana pt margine
EDGE_GRADIENT_MIN_FRAC = 0.25    # gradient minim relativ la percentila 95 in zona pleurei
EDGE_SMOOTH_KERNEL = 7
EDGE_FALLBACK_HALF_PX = 3        # daca gradient sub prag, foloseste fallback fix de 3 px

# RAPORTARE (NU constrangere) — coloane peste pragul asta sunt marcate ca "suspecte"
EDGE_THICKNESS_PLAUSIBLE_MM = 1.2   # peste asta, conturul e suspect anatomic
EDGE_THICKNESS_HARD_LIMIT_MM = 3.0  # peste asta sigur e zgomot, restrange (ultim safety)

# Constraint-uri de continuitate (anti-deviatii)
EDGE_LATERAL_PENALTY = 0.05      # penalty pe pixel de deviatie fata de coloana vecina
EDGE_OUTLIER_NEIGHBORHOOD = 20   # cate coloane vecine consideram ca "context"
EDGE_OUTLIER_THRESH_PX = 4       # daca margine deviaza >4 px de la mediana vecinilor = outlier
EDGE_REFINE_PASSES = 2           # cate iteratii de refinare (smoothing iterativ)

# Parametri pastrati pt fallback / metoda veche bazata pe intensitate
BAND_INTENSITY_FRAC = 0.75
BAND_MAX_HALF_WIDTH_MM = 1.0
...
def _dp_optimal_path(cost_map, lambda_penalty, max_jump, anchors=None,
    ...
    Returneaza:
      ...
      dp_cost : np.array(W,) costul cumulativ pe calea optima
    """
    H, W = cost_map.shape
...
def _find_band_edges_gradient(column, gradient_col, y_center, search_half_px,
                                gradient_thresh, fallback_half_px=3):
    """
    Detecteaza marginile benzii pleurale folosind gradient vertical de intensitate.

    Pleura are tranzitii abrupte la margini:
      - Sus: muschi (intunecat) -> pleura (luminos) = gradient POZITIV mare
      - Jos: pleura (luminos) -> aer (intunecat) = gradient NEGATIV mare

    Args:
      column            : intensitate pe coloana (1D array, valori in [0,1])
      gradient_col      : gradient vertical pe coloana
      y_center          : pozitia centrala (de pe pleura_smooth)
      search_half_px    : cat cautam in sus/jos
      gradient_thresh   : prag minim absolut pt gradient (ca sa fie valid)
      fallback_half_px  : daca niciun gradient nu trece pragul, foloseste +/-fallback px
    ...
    # ─ Margine SUS: caut gradient POZITIV maxim
    # Fereastra: [y_center - search_half_px, y_center + 1] (inclusiv +1 pt cazul cand pic = centru)
    y_top_min = max(0, y_center - search_half_px)
    y_top_max = min(H, y_center + 1)

    if y_top_max > y_top_min:
        grad_search_top = gradient_col[y_top_min:y_top_max]
        max_pos_grad = grad_search_top.max()
        if max_pos_grad >= gradient_thresh:
            y_top = y_top_min + int(np.argmax(grad_search_top))
        else:
            # Fallback: folosesc pozitie fixa relativa la centru
            y_top = max(0, y_center - fallback_half_px)
    else:
        y_top = y_center

    # ─ Margine JOS: caut gradient NEGATIV maxim (= -gradient pozitiv maxim)
    # Fereastra: [y_center, y_center + search_half_px + 1]
    y_bot_min = max(0, y_center)
    y_bot_max = min(H, y_center + search_half_px + 1)

    if y_bot_max > y_bot_min:
        grad_search_bot = gradient_col[y_bot_min:y_bot_max]
        max_neg_grad = (-grad_search_bot).max()
        if max_neg_grad >= gradient_thresh:
            y_bot = y_bot_min + int(np.argmin(grad_search_bot))
        else:
            # Fallback fix
            y_bot = min(H - 1, y_center + fallback_half_px)
    else:
        y_bot = y_center

    # Constraint: top <= centru <= bot, si banda sa aiba grosime minim 2px
    if y_top > y_center:
        y_top = y_center
    if y_bot < y_center:
        y_bot = y_center
    if y_bot - y_top < 2:
        # Fortez minim 2 px grosime
        y_top = max(0, y_center - 1)
        y_bot = min(H - 1, y_center + 1)

    return y_top, y_bot


def _find_edge_with_continuity(gradient_col, y_center, search_half_px,
                                  gradient_thresh, prev_y, lateral_penalty,
                                  is_top_edge, fallback_half_px):
    """
    Detecteaza o margine (sus SAU jos) folosind:
      - gradient maxim in fereastra de cautare
      - penalizare proportionala cu distanta de la pozitia coloanei vecine

    Score(y) = |gradient(y)| - lateral_penalty * |y - prev_y|

    Returneaza y al marginii.
    """
    H = len(gradient_col)
    y_center = int(y_center)

    if is_top_edge:
        y_lo = max(0, y_center - search_half_px)
        y_hi = min(H, y_center + 1)
    else:
        y_lo = max(0, y_center)
        y_hi = min(H, y_center + search_half_px + 1)

    if y_hi <= y_lo:
        return y_center

    grad_window = gradient_col[y_lo:y_hi]
    if is_top_edge:
        grad_signed = grad_window  # max pozitiv
    else:
        grad_signed = -grad_window  # max negativ

    valid_grad = grad_signed >= gradient_thresh

    if not valid_grad.any():
        if is_top_edge:
            return max(0, y_center - fallback_half_px)
        else:
            return min(H - 1, y_center + fallback_half_px)

    ys_in_window = np.arange(y_lo, y_hi)
    if prev_y is not None:
        penalty = lateral_penalty * np.abs(ys_in_window - prev_y)
    else:
        penalty = np.zeros_like(grad_signed)

    score = grad_signed - penalty
    score[~valid_grad] = -np.inf
    ...
    return y_lo + best_idx


def _refine_edges_outlier_replace(edges, neighborhood, threshold_px):
    """
    Pentru fiecare punct de margine, calculeaza mediana vecinilor (±neighborhood).
    Daca punctul deviaza > threshold_px de la mediana, il inlocuieste cu mediana.
    """
    n = len(edges)
    if n < 3:
        return edges.copy()
    ...
    for i in range(n):
        ...
        local_median = np.median(edges[lo:hi])
        if abs(edges[i] - local_median) > threshold_px:
            refined[i] = local_median
...
def _find_band_edges(column, y_center, intensity_frac, max_half_width_px):
    """
    [METODA VECHE — pastrata pt compatibilitate / comparatie]
...
def ExtractPleuralContour(result, img_denoised, one_pixel_mm, edge_method='gradient'):
    """
    ...
    Pentru fiecare segment continuu de pleura (intre intreruperi):
      - Detecteaza marginile sus/jos folosind gradient vertical (edge_method='gradient')
        sau threshold de intensitate (edge_method='intensity', metoda veche)
      - Aplica smoothing median pe margini (continuitate intre coloane vecine)
      - Construieste contur inchis

    Parametri:
      result          : dict de la ExtractPleuralLine
      img_denoised    : imaginea uint8 (post remove_noise)
      one_pixel_mm    : rezolutia in mm/px
      edge_method     : 'gradient' (recomandat) sau 'intensity' (legacy)

    Returneaza dict ca inainte: segments, interruptions, top_edges, bottom_edges,
    contours_cv, thickness_mm
    """
    pleura_smooth = result['pleura_y_smooth']
    enhanced = result['enhanced']  # float [0,1]
    H, W = enhanced.shape

    min_gap_px = max(1, _mm_to_px(INTERRUPT_MIN_GAP_MM, one_pixel_mm))

    # Parametri specifici pt fiecare metoda
    if edge_method == 'gradient':
        search_half_px = max(2, _mm_to_px(EDGE_SEARCH_HALF_MM, one_pixel_mm))
        # Calculeaza gradient vertical pe toata imaginea
        gradient = cv2.Sobel(enhanced, cv2.CV_32F, dx=0, dy=1, ksize=3)

        # Prag adaptiv DIN ZONA PLEUREI (nu din toata imaginea):
        # - calculam zona y unde se afla majoritatea inlierilor
        # - folosim percentila 95 (nu max — un punct extrem nu trebuie sa strice pragul)
        valid_pleura = ~np.isnan(pleura_smooth)
        if valid_pleura.sum() > 0:
            y_pleura_min = int(np.nanmin(pleura_smooth)) - search_half_px
            y_pleura_max = int(np.nanmax(pleura_smooth)) + search_half_px
            y_pleura_min = max(0, y_pleura_min)
            y_pleura_max = min(H, y_pleura_max)
            grad_zone = gradient[y_pleura_min:y_pleura_max, :]
            grad_p95 = np.percentile(np.abs(grad_zone), 95)
        else:
            grad_p95 = max(np.abs(gradient).max(), 1e-6)

        gradient_thresh = EDGE_GRADIENT_MIN_FRAC * grad_p95
        print(f"[ExtractPleuralContour] Gradient (in zona pleurei): "
              f"p95 abs={grad_p95:.3f}, thresh={gradient_thresh:.3f}")
    else:
        max_half_width_px = max(2, _mm_to_px(BAND_MAX_HALF_WIDTH_MM, one_pixel_mm))
        gradient = None
        gradient_thresh = None
        search_half_px = None

    # ─ 1. Identifica segmente continue si intreruperi (la fel ca inainte)
    ...
    # ─ 2. Pentru fiecare segment, detecteaza margini + aplica smoothing
    top_edges = []
    bottom_edges = []
    contours_cv = []
    ...
    for x_start, x_end in segments:
        ...
        # ─ Pass 1: detectie cu gradient + penalty lateral pt continuitate
        for x in range(x_start, x_end + 1):
            ...
            if edge_method == 'gradient':
                gradient_col = gradient[:, x]
                y_top = _find_edge_with_continuity(
                    gradient_col, y_c, search_half_px, gradient_thresh,
                    prev_top, EDGE_LATERAL_PENALTY,
                    is_top_edge=True, fallback_half_px=EDGE_FALLBACK_HALF_PX,
                )
                y_bot = _find_edge_with_continuity(
                    gradient_col, y_c, search_half_px, gradient_thresh,
                    prev_bot, EDGE_LATERAL_PENALTY,
                    is_top_edge=False, fallback_half_px=EDGE_FALLBACK_HALF_PX,
                )
                prev_top = y_top
                prev_bot = y_bot
            else:
            ...
            seg_xs.append(x)
            seg_top_pass1.append(y_top)
            seg_bot_pass1.append(y_bot)

        if len(seg_xs) < 2:
            continue

        seg_xs_arr = np.array(seg_xs)
        seg_top_arr = np.array(seg_top_pass1, dtype=np.float64)
        seg_bot_arr = np.array(seg_bot_pass1, dtype=np.float64)

        # ─ Pass 2+: refinare iterativa — inlocuim outlierii cu mediana vecinilor
        if edge_method == 'gradient':
            for refine_iter in range(EDGE_REFINE_PASSES):
                seg_top_arr = _refine_edges_outlier_replace(
                    seg_top_arr, EDGE_OUTLIER_NEIGHBORHOOD, EDGE_OUTLIER_THRESH_PX
                )
                seg_bot_arr = _refine_edges_outlier_replace(
                    seg_bot_arr, EDGE_OUTLIER_NEIGHBORHOOD, EDGE_OUTLIER_THRESH_PX
                )

        # ─ Pass final: smoothing median pt continuitate vizuala
        ksize = min(EDGE_SMOOTH_KERNEL, len(seg_xs_arr))
        if ksize >= 3 and ksize % 2 == 1:
            seg_top_smooth = medfilt(seg_top_arr, kernel_size=ksize)
            seg_bot_smooth = medfilt(seg_bot_arr, kernel_size=ksize)
        else:
            seg_top_smooth = seg_top_arr
            seg_bot_smooth = seg_bot_arr

        # ─ Constraint final geometric (DOAR safety extrem, NU forteaza grosime tipica)
        hard_limit_px = _mm_to_px(EDGE_THICKNESS_HARD_LIMIT_MM, one_pixel_mm)
        for i, x in enumerate(seg_xs_arr):
            y_c = pleura_smooth[x]
            if seg_top_smooth[i] > y_c:
                seg_top_smooth[i] = y_c
            if seg_bot_smooth[i] < y_c:
                seg_bot_smooth[i] = y_c
            # Banda minim 2 px
            if seg_bot_smooth[i] - seg_top_smooth[i] < 2:
                seg_top_smooth[i] = max(0, y_c - 1)
                seg_bot_smooth[i] = min(H - 1, y_c + 1)

            # SAFETY EXTREM: peste 3mm sigur e zgomot/contaminare cu alte structuri
            # Restrange simetric la limita absoluta. NU e o "corectie cosmetica" —
            # 3mm e dincolo de orice valoare anatomic plauzibila.
            current_thickness = seg_bot_smooth[i] - seg_top_smooth[i]
            if current_thickness > hard_limit_px:
                half = hard_limit_px // 2
                seg_top_smooth[i] = max(0, y_c - half)
                seg_bot_smooth[i] = min(H - 1, y_c + half)

            thickness_mm[x] = (seg_bot_smooth[i] - seg_top_smooth[i]) * one_pixel_mm

        top_arr = np.column_stack([seg_xs_arr, seg_top_smooth.astype(np.int32)])
        bot_arr = np.column_stack([seg_xs_arr, seg_bot_smooth.astype(np.int32)])

        top_edges.append(top_arr)
        bottom_edges.append(bot_arr)

        cnt = np.concatenate([top_arr, bot_arr[::-1]], axis=0)
        cnt_cv = cnt.reshape(-1, 1, 2).astype(np.int32)
        contours_cv.append(cnt_cv)

    # ─ 3. Statistici + RAPORT DE CALITATE
    valid_th = ~np.isnan(thickness_mm)
    print(f"\n[ExtractPleuralContour] (edge_method={edge_method})")
    ...
    if valid_th.sum() > 0:
        print(f"  Grosime pleura (mm) — mean: {thickness_mm[valid_th].mean():.2f}, "
              f"median: {np.median(thickness_mm[valid_th]):.2f}, "
              f"std: {thickness_mm[valid_th].std():.2f}")
        print(f"  Grosime — min: {thickness_mm[valid_th].min():.2f}, "
              f"max: {thickness_mm[valid_th].max():.2f}")

        # RAPORT DE CALITATE — ce procent din contur depaseste pragul anatomic plauzibil
        n_total = valid_th.sum()
        n_suspect = (thickness_mm[valid_th] > EDGE_THICKNESS_PLAUSIBLE_MM).sum()
        n_in_range = ((thickness_mm[valid_th] >= 0.3) &
                      (thickness_mm[valid_th] <= EDGE_THICKNESS_PLAUSIBLE_MM)).sum()
        n_thin = (thickness_mm[valid_th] < 0.3).sum()

        print(f"\n  [Raport calitate contur]")
        print(f"    Coloane in interval anatomic plauzibil (0.3-{EDGE_THICKNESS_PLAUSIBLE_MM}mm): "
              f"{n_in_range}/{n_total} ({100*n_in_range/n_total:.1f}%)")
        print(f"    Coloane prea subtiri (<0.3mm, posibila colapsare): "
              f"{n_thin}/{n_total} ({100*n_thin/n_total:.1f}%)")
        print(f"    Coloane suspecte (>{EDGE_THICKNESS_PLAUSIBLE_MM}mm, posibila contaminare cu "
              f"structuri vecine): {n_suspect}/{n_total} ({100*n_suspect/n_total:.1f}%)")

        # Identifica zonele cu grosime suspecta (intervale de coloane consecutive)
        if n_suspect > 0:
            suspect_mask = (thickness_mm > EDGE_THICKNESS_PLAUSIBLE_MM)
            suspect_xs = np.where(suspect_mask)[0]
            # Grupeaza in intervale consecutive
            if len(suspect_xs) > 0:
                gaps = np.where(np.diff(suspect_xs) > 5)[0]
                splits = np.split(suspect_xs, gaps + 1)
                print(f"    Intervale suspecte (>10 col consecutiv):")
                for j, group in enumerate(splits):
                    if len(group) >= 10:
                        avg_thick = thickness_mm[group].mean()
                        print(f"      x={group[0]}-{group[-1]} "
                              f"(grosime medie: {avg_thick:.2f}mm)")
...
def plot_contour_zoom(contour_result, img_denoised, one_pixel, pad_mm=5.0, save_path=None):
    ...
    for ax, img, title in [
        ...
        # Contururile per segment, separat colorate sus/jos + evidentiez zonele suspecte
        for i, (top_arr, bot_arr) in enumerate(zip(contour_result['top_edges'],
                                                    contour_result['bottom_edges'])):
            # Linia de baza (verde-galben din colormap)
            ax.plot(top_arr[:, 0], top_arr[:, 1], color=colors_seg[i],
                    linewidth=2, label=f'Segment {i+1} (sus)')
            ax.plot(bot_arr[:, 0], bot_arr[:, 1], color=colors_seg[i],
                    linewidth=2, linestyle='--', label=f'Segment {i+1} (jos)')

            # Suprapun GROS marcaj rosu pe coloanele unde grosimea e suspecta
            for j in range(len(top_arr)):
                xx = top_arr[j, 0]
                if not np.isnan(thickness[xx]) and thickness[xx] > EDGE_THICKNESS_PLAUSIBLE_MM:
                    ax.plot([xx, xx], [top_arr[j, 1], bot_arr[j, 1]],
                            color='red', linewidth=0.5, alpha=0.4)

        # Linii verticale pentru intreruperi
        for i, (xs_i, xe_i, wmm) in enumerate(contour_result['interruptions']):
            if xs_i < x_max and xe_i > x_min:
...
def plot_contour_clean(contour_result, img_denoised, save_path=None):
    ...
    if save_path:
        ...
        print(f"[plot_contour_clean] Salvat in: {save_path}")
        plt.close()
    else:
        plt.show(block=False)
...
def batch_process(input_dir='./ORIGINAL_IMAGES', output_dir='./CONTUR_OUTPUT',
                  save_only=True, max_display=0):
    """
    Proceseaza toate imaginile .jpg din input_dir si salveaza conturul ca PNG.

    Args:
      input_dir   : folder cu imaginile sursa
      output_dir  : folder unde se salveaza PNG-urile cu contur
      save_only   : True = doar salveaza, False = afiseaza si ferestre
      max_display : daca save_only=False, afiseaza primele max_display imagini

    Pentru fiecare imagine:
      - PixelConverter, CropBorder, remove_noise
      - ExtractPleuralLine (DP) + ExtractPleuralContour (gradient)
      - Salveaza conturul curat ca PNG cu acelasi nume
    """
    import os
    import glob

    # Creaza folder de output daca nu exista
    os.makedirs(output_dir, exist_ok=True)

    # Listeaza toate .jpg din input
    pattern = os.path.join(input_dir, '*.jpg')
    image_paths = sorted(glob.glob(pattern))

    if not image_paths:
        print(f"[batch_process] Niciun fisier .jpg in {input_dir}")
        return

    print(f"[batch_process] Procesez {len(image_paths)} imagini din {input_dir}")
    print(f"[batch_process] Salvare in {output_dir}\n")
    ...
    for i, img_path in enumerate(image_paths):
        ...
        output_path = os.path.join(output_dir, f"{base_name}_contur.png")

        print(f"[{i+1}/{len(image_paths)}] {img_name}...", end=' ')
    ...
    print(f"\n{'═'*60}")
    print(f"  REZUMAT BATCH")
    print(f"{'═'*60}")
    print(f"  Reusite: {success_count}/{len(image_paths)}")
    print(f"  Esecuri: {fail_count}/{len(image_paths)}")
    if failed_images:
        print(f"\n  Imagini cu erori:")
...
def main():
    ...
    # FEREASTRA NOUA: plot curat — doar imagine + contur (NDA local)
    plot_contour_clean(contour_result, img_denoised)
